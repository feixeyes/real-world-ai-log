...
- 之前用 Codex 更久，觉得很多经验可能相通，但对两者具体差异的细节还缺一个系统性的认识
...
### 1.0 先补一块：Codex vs Claude Code 的“差异细节”（你需要的是这张地图）
你说“经验相通，但缺一个系统性的差异认识”，我理解成：
- 不是不知道怎么用，而是不知道**什么时候该用哪种姿势**

先给一个我目前的粗地图（不求完美，但够你开始做对比实验）：

- **交互颗粒度**
  - Codex：更像“执行器”，你把任务拆得越细，它越稳
  - Claude Code：更像“同事”，它会主动把任务串成流程，但也更容易在你没写清 DoD 时跑偏

- **工具链编排能力**
  - Codex：你通常需要更明确地告诉它“先做 A 再做 B”
  - Claude Code：更擅长自己去读文件、跑命令、再回来解释（前提是你给了可验收目标）

- **读代码/建模能力（2/3 类任务的核心）**
  - Codex：读得快，但更依赖你给的定位线索
  - Claude Code：更愿意先建立“系统模型”（入口/数据流/边界），适合你现在的主战场

- **风险**
  - Codex：容易“只做你说的那一点”，遗漏周边（需要你补充验收）
  - Claude Code：容易“顺手把周边也改了”（需要你写清禁止项 + 变更范围）

下面的 4 类技巧，基本都是在解决这张地图里的失败模式。
...
这一类我建议你把指令写得更“工程化”，不要怕啰嗦。因为你主要在做 2/3 类任务：
- 写新功能：需要它跑通验证链路
- 读代码：需要它把证据（文件/调用链/关键函数）摆出来
...
我自己会用一个极简记录表（直接贴在笔记里就行）：

- 任务：________
- Baseline（不加技巧）结果：可运行率__ / 返工__ / 轮次__ / 质量__
- +技巧1：________ 结果：__ / __ / __ / __ 备注：________
- +技巧2：________ 结果：__ / __ / __ / __ 备注：________
- +技巧3：________ 结果：__ / __ / __ / __ 备注：________

这样你 10 分钟就能把“感觉”变成“可比较的数据”。
