...
这背后其实是同一个逻辑：把不确定性切碎，变成一个个可验收的步骤。
...
## 2) 通道与会话：为什么它能同时处理 Discord/WhatsApp 而不乱
我们这次实践里，一个直接收益是：
- Discord 跑着不影响 WhatsApp 接入
- WhatsApp 可以设置 allowlist，只允许你的号码，群禁用

这种“多通道不互相污染”的能力，来自一个朴素但很强的设计：
- **channel 负责收发**
- **session 负责上下文**
- 回复默认“回到原通道”，不需要你额外操心

对比一下 Happy + Claude Code 的接力方案：
- 它当然也能做很多事
- 但跨设备/跨网络时，你要自己管理“这一步产物在哪、下一步去哪执行、怎么把结果发回来”

OpenClaw 把这部分从“人的记忆/手工”变成了系统机制。

---

## 3) 工具系统：能力来自边界，而不是模型更聪明
...
我们这次的实际链路里，关键工具非常具体：
- Playwright（抓取 X / 操作 WeChat）
- message（把结果投递回 WhatsApp）
- cron（让“每天 07:00”变成默认行为）

工具边界清晰，就意味着：
- 失败不会“玄学化”，而是变成可定位的错误（cookie 失效、找不到按钮、超时等）
- 你可以逐步增强系统，而不是推倒重来

---

## 4) 定时任务（cron）：从“提醒我”到“自动交付”的关键差异
这是我们今天最值得写下来的坑：
...
## 6) 我们从 OpenClaw 借走的 5 个设计模式（本篇结论）
